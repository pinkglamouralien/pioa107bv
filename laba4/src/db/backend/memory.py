from src.db.backend.errors import (DuplicateRecordError, RecordNotFoundError)

class Guest:
    def __init__(self, guest_id: int, name: str, phone: str):
        self.id = guest_id
        self.name = name
        self.phone = phone
...
class Dish:
    def __init__(self, dish_id: int, name: str, price: float):
        self.id = dish_id
        self.name = name
        self.price = price
...
class Order:
    def __init__(self, order_id: int, guest_id: int, dish_id: int, quantity: int):
        self.id = order_id
    ...
    def __str__(self):
        return f"Order (id = {self.id}, guest_id = {self.guest_id}, dish_id = {self.dish_id}, quantity = {self.quantity})"
...
class RestaurantDB:
    ...
    def create_guest(self, guest_id: int, name: str, phone: str):
        ...
        guest = Guest(guest_id, name.strip(), phone.strip())
        self.guests.append(guest)
        return guest

    def read_guests(self, name: str | None = None):
        if name is None:
            return self.guests
    ...
    def sort_guests(self, field: str, reverse=False):
        return sorted(self.guests, key=lambda guest: getattr(guest, field), reverse=reverse)
    ...
    def create_dish(self, dish_id: int, name: str, price: float):
        ...
        dish = Dish(dish_id, name.strip(), price)
    ...
    def create_order(self, order_id: int, guest_id: int, dish_id: int, quantity: int):
        ...
        order = Order(order_id, guest_id, dish_id, quantity)
